...
> **Owner tier on `skillcorner`:** beyond the public open data, the `skillcorner` provider also carries **restricted** matches served only to the owner bearer token (`visibility=private`); public consumers see only the redistributed open data. These matches are stored in the canonical columnar format — role-aligned keys `tracking` (nested Parquet/zstd), `events` and `physical` (Parquet/zstd), and `metadata`, with a per-match `format_version` marker; `freeze_frames` is not carried (it is a redundant slice of tracking). Restricted data is access-gated, never redistributed. See [ADR 0009](docs/decisions/0009-restricted-tier-under-existing-public-provider.md) and [ADR 0011](docs/decisions/0011-canonical-owner-tier-skillcorner-format.md).

> **Owner-tier provider `statsbomb`:** served only to the owner bearer token (`visibility=private`) — there is **no public tier** for this provider — and carrying **two source families** under one slug, distinguished by `provenance`. The **commercial** StatsBomb 360 club delivery is `provenance="original"` (ADR 0010). The **open-data tournaments** family (`provenance="redistributed"`) re-hosts StatsBomb's public open-data 360 corpus for six complete tournaments (292 matches: FIFA World Cup 2022, Women's World Cup 2023, UEFA Euro 2020/2024, UEFA Women's Euro 2022/2025) — freely accessible but **not freely redistributable** under the StatsBomb Public Data User Agreement, so it lives at the owner tier as a single-user (party-of-one) re-host, never on a public tier (ADR 0012). Both families use the same faithful-feed role-aligned keys: `events`, `freeze_frames`, `roster`, `metadata`. There is deliberately no `tracking` artifact — StatsBomb 360 supplies event-moment freeze frames, not continuous tracking. Restricted data is access-gated, never redistributed to third parties. See [ADR 0010](docs/decisions/0010-faithful-feed-mimicry.md) and [ADR 0012](docs/decisions/0012-statsbomb-open-data-second-source-family.md).
