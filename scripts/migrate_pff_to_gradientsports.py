"""Migrate S3 data from pff/ prefix to gradientsports/ prefix.
...
Usage:
    python scripts/migrate_pff_to_gradientsports.py --bucket karstenskyt-pining-for-the-data [--dry-run]
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Migrate S3 data from pff/ to gradientsports/")
    ...
    try:
        ...
        if matches_data.get("provider") == "pff":
            matches_data["provider"] = "gradientsports"
            if args.dry_run:
                print(f"  [dry-run] UPDATE {matches_key}: provider pff -> gradientsports")
            else:
                s3.put_object(
                    Bucket=args.bucket,
                    Key=matches_key,
                ...
                print(f"  UPDATED {matches_key}: provider pff -> gradientsports")
    ...
    for players_key in [f"{new_prefix}players.json", f"{new_prefix}_private/players.json"]:
        try:
            ...
            if players_data.get("provider") == "pff":
                players_data["provider"] = "gradientsports"
                if args.dry_run:
                    print(f"  [dry-run] UPDATE {players_key}: provider pff -> gradientsports")
                else:
                    s3.put_object(
                        Bucket=args.bucket,
                        Key=players_key,
                    ...
                    print(f"  UPDATED {players_key}: provider pff -> gradientsports")
        except s3.exceptions.NoSuchKey:
            pass
    ...
    try:
        ...
        if "pff" in providers_list:
            providers_list = ["gradientsports" if p == "pff" else p for p in providers_list]
            providers_data["providers"] = providers_list
            if args.dry_run:
                print("  [dry-run] UPDATE providers.json: pff -> gradientsports in providers list")
            else:
                s3.put_object(
                    Bucket=args.bucket,
                    Key="providers.json",
                ...
                print("  UPDATED providers.json: pff -> gradientsports in providers list")
    except s3.exceptions.NoSuchKey:
